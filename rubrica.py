...
"""def cerca(rubrica):
  daCercare=input("Inserisci il cognome e il nome da cercare")
  if(daCercare in rubrica.keys()):
      return"""
...
"""def modifica(rubrica):
    daModificare=input("Inserisci il cognome e il nome per modificare il contatto:")
    if(daModificare in rubrica.keys()):
        trovato=rubrica[daModificare]
        for CognomeNome in rubrica[daModificare]:
